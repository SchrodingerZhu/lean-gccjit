...
extern "C" LEAN_EXPORT lean_obj_res lean_gcc_jit_context_new_struct_constructor(
  ...
  if (values_len > INT_MAX) {
    auto error = lean_mk_io_error_invalid_argument(
        EINVAL, lean_mk_string("too many values"));
  ...
  if (fields_ctor == 1) {
    ...
    if (fields_len != values_len) {
      auto error = lean_mk_io_error_invalid_argument(
          EINVAL, lean_mk_string("fields and values have different lengths"));
      return lean_io_result_mk_error(error);
    }
    result = with_allocation<gcc_jit_rvalue *>(values_len, [=](auto *vbuffer) {
      return with_allocation<gcc_jit_field *>(fields_len, [=](auto *fbuffer) {
        for (size_t i = 0; i < fields_len; i++) {
          fbuffer[i] =
              unwrap_pointer<gcc_jit_field>(lean_to_array(fields_)->m_data[i]);
        }
        for (size_t i = 0; i < values_len; i++) {
          vbuffer[i] =
              unwrap_pointer<gcc_jit_rvalue>(lean_to_array(values)->m_data[i]);
        }
        return gcc_jit_context_new_struct_constructor(
            context, location, type, fields_len, fbuffer, vbuffer);
      });
    });
  } else {
    result = with_allocation<gcc_jit_rvalue *>(values_len, [=](auto *vbuffer) {
      ...
      return gcc_jit_context_new_struct_constructor(
          context, location, type, num_values, nullptr, vbuffer);
    });
  }
  return map_notnull(result, wrap_pointer<gcc_jit_rvalue>,
                     "failed to create struct constructor");
}
} // namespace lean_gccjit